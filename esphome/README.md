# ESPHome: direct RWB1 Bluetooth telemetry

`rwb1_ble.yaml` is the ready-to-use ESP32-C3 configuration. It discovers the
...
2. Copy `rwb1_ble.yaml` over the generated device configuration.
...
`rwb1_ble_diagnostic.yaml` is a small read-only build with no Wi-Fi, API, OTA,
